...
elif cfg.tts_module == "vits":
    from tts.vits import Speech
else:
...
class FeiFei:
    def __init__(self):
        ...
        self.W = np.array([0.0, 0.6, 0.1, 0.7, 0.3, 0.0, 0.0, 0.0]).reshape(
            -1, 1
        )  # 适应模型变量矩阵
        self.wsParam = None
        self.wss = None
        ...
        if self.__running:
            try:
                ...
                if index == 1:  # 语音文字交互
                    ...
                    # 同步用户问题到数字人
                    if wsa_server.get_instance().is_connected(
                        interact.data.get("user")
                    ):
                        content = {
                            "Topic": "Unreal",
                            "Data": {"Key": "question", "Value": interact.data["msg"]},
                    ...
                    if (
                        member_db.new_instance().is_username_exist(username)
                        == "notexists"
                    ):
                            ...
                            wsa_server.get_web_instance().add_cmd(
                                {
                                    "panelMsg": "思考中...",
                                    "Username": username,
                                    "robot": f"http://{cfg.fay_url}:5000/robot/Thinking.jpg",
                                }
                            )
                        if wsa_server.get_instance().is_connected(username):
                            content = {
                                "Topic": "Unreal",
                                "Data": {"Key": "log", "Value": "思考中..."},
                                "Username": username,
                                "robot": f"http://{cfg.fay_url}:5000/robot/Thinking.jpg",
                            }
                            wsa_server.get_instance().add_cmd(content)
                        ...
                        wsa_server.get_web_instance().add_cmd(
                            {
                                "panelMsg": text,
                                "Username": username,
                                "robot": f"http://{cfg.fay_url}:5000/robot/Speaking.jpg",
                            }
                        )
                        if type == "qa":
                            wsa_server.get_web_instance().add_cmd(
                    ...
                    if len(textlist) > 1:
                        ...
                        while i < len(textlist):
                            content_db.new_instance().add_content(
                                "fay", "speak", textlist[i]["text"], username, uid
                            )
                            if wsa_server.get_web_instance().is_connected(username):
                                wsa_server.get_web_instance().add_cmd(
                                    {
                    ...
                    util.printInfo(
                        1,
                        interact.data.get("user"),
                        "({}) {}".format(self.__get_mood_voice(), text),
                    )
                    if wsa_server.get_instance().is_connected(username):
                        content = {
                            "Topic": "Unreal",
        ...
        if interact.interact_type == 1:
            try:
                if cfg.ltp_mode == "cemotion":
                    ...
                    if (
                        str(cfg.baidu_emotion_api_key) == ""
                        or str(cfg.baidu_emotion_app_id) == ""
                        or str(cfg.baidu_emotion_secret_key) == ""
                    ):
                        self.mood = 0
                    else:
    ...
    def say(self, interact, text):
        try:
            ...
            if audio_url is not None:
                ...
                config_util.config["interact"]["playSound"]
                or wsa_server.get_instance().is_connected(interact.data.get("user"))
                or self.__is_send_remote_device_audio(interact)
            ):  # tts
                util.printInfo(1, interact.data.get("user"), "合成音频...")
                tm = time.time()
                result = self.sp.to_sample(
                    text.replace("*", ""), self.__get_mood_voice()
                )
                util.printInfo(
                    1,
                    interact.data.get("user"),
                    "合成音频完成. 耗时: {} ms 文件:{}".format(
            ...
            if result is not None:
                MyThread(
                    target=self.__process_output_audio, args=[result, interact, text]
                ).start()
                return result
            else:
                if wsa_server.get_web_instance().is_connected(
                    interact.data.get("user")
                ):
                    wsa_server.get_web_instance().add_cmd(
                        {
    ...
    def __send_remote_device_audio(self, file_url, interact):
        ...
        for key, value in fay_booter.DeviceInputListenerDict.items():
            if (
                value.username == interact.data.get("user") and value.isOutput
                ...
                try:
                    ...
                    while data:
                        ...
                        time.sleep(0.0001)
                    value.deviceConnector.send(
                        b"\x08\x07\x06\x05\x04\x03\x02\x01\x00"
                    )  # 发送音频结束标志
                    util.printInfo(
                        1, value.username, "远程音频发送完成：{}".format(total)
                    )
    ...
    def __process_output_audio(self, file_url, interact, text):
        try:
            ...
            # 推送远程音频
            MyThread(
                target=self.__send_remote_device_audio, args=[file_url, interact]
            ).start()
            ...
            if wsa_server.get_instance().is_connected(interact.data.get("user")):
                content = {
                    ...
                    "Data": {
                        ...
                        "HttpValue": f"http://{cfg.fay_url}:5000/audio/"
                ...
                wsa_server.get_instance().add_cmd(content)
                util.printInfo(
                    1, interact.data.get("user"), "数字人接口发送音频数据成功"
                )
