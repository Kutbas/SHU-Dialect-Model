...
new Vue({
    ...
    methods: {
        ...
        handleResize() {
            const settingContainer = this.$el.querySelector('.setting_fay');
            if (settingContainer) {
                const headerHeight = this.$el.querySelector('.setting_fay_top').offsetHeight;
                const footerHeight = this.$el.querySelector('.setting_autoplay').offsetHeight;
                settingContainer.style.height = `calc(100vh - ${headerHeight + footerHeight}px)`;
            }
        }
    },
    computed: {
        ...
        isSettingPage() {
            return window.location.pathname === '/setting';
        }
    },
    beforeDestroy() {
